...
import { Save, Check, Instagram, Twitter, Linkedin, Globe, Feather, Camera, Facebook } from 'lucide-react'
...
export default function DashboardPage() {
    ...
    const handleProfilePic = async (e) => {
        ...
        try {
            ...
            await supabase.from('applications').update({ profile_pic: urlData.publicUrl }).eq('id', artist.id)
            setArtist({ ...artist, profile_pic: urlData.publicUrl })
    ...
    return (
        <div className="page-enter pt-24 pb-28 px-4">
            <div className="max-w-3xl mx-auto">
                ...
                {/* Profile Picture — SEPARATE from artwork */}
                <div className="rounded-sm p-7 mb-6" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)' }}>
                    <h2 className="font-display text-2xl font-light mb-2" style={{ color: 'var(--ghost)' }}>Profile Picture</h2>
                    <p className="text-sm mb-5" style={{ color: 'rgba(234,230,242,0.35)' }}>
                        This is your avatar — separate from your submitted artwork.
                    </p>
                    <div className="flex items-center gap-6">
                        <div className="w-20 h-20 rounded-full overflow-hidden relative flex-shrink-0"
                            ...
                            {artist.profile_pic ? (
                                <img src={artist.profile_pic} alt={artist.name} className="w-full h-full object-cover" />
                            ) : artist.image_url ? (
                                <img src={artist.image_url} alt={artist.name} className="w-full h-full object-cover" />
                            ) : (
                                <div className="w-full h-full flex items-center justify-center"
                                    style={{ background: 'rgba(0,240,255,0.1)' }}>
                                    <span className="font-display text-3xl font-light" style={{ color: 'var(--cyan)' }}>
                                        {artist.name?.[0]}
                                    </span>
                                </div>
                            )}
                        ...
                        <div>
                            <p className="text-sm mb-3" style={{ color: 'rgba(234,230,242,0.4)' }}>
                                Upload a profile photo. This will show on your chamber and in chats.
                            </p>
                            <label className="btn-velvet cursor-pointer inline-flex">
                                <Camera size={13} />
                                <span>{uploadingPic ? 'Uploading...' : 'Change Profile Picture'}</span>
                                <input type="file" className="hidden" accept="image/*" onChange={handleProfilePic} disabled={uploadingPic} />
                            </label>
                        </div>
                    </div>
                </div>
                ...
                <div className="rounded-sm p-7 mb-6" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)' }}>
                    <h2 className="font-display text-2xl font-light mb-2" style={{ color: 'var(--ghost)' }}>Submitted Artwork</h2>
                    <p className="text-sm mb-4" style={{ color: 'rgba(234,230,242,0.35)' }}>
                        Your original application artwork. To add more works, visit your Artist Chamber.
                    </p>
                    {artist.image_url ? (
                        <div className="flex items-center gap-4">
                            <img src={artist.image_url} alt="artwork" className="w-20 h-20 object-cover rounded-sm" />
                            <div>
                                <p className="text-sm" style={{ color: 'var(--ghost)' }}>{artist.medium}</p>
                                <p className="text-xs mt-1" style={{ color: 'rgba(234,230,242,0.3)' }}>{artist.mood}</p>
                                <a href={`/artist/${artist.username}`} className="text-xs mt-2 block" style={{ color: 'var(--cyan)' }}>
                                    View Chamber & Add More →
                                </a>
                            </div>
                        </div>
                    ) : (
                        <p className="text-sm" style={{ color: 'rgba(234,230,242,0.3)' }}>No artwork submitted yet.</p>
                    )}
                </div>

                {/* Bio & Social Links */}
                <div className="rounded-sm p-7 mb-6" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)' }}>
                    <h2 className="font-display text-2xl font-light mb-6" style={{ color: 'var(--ghost)' }}>Profile Info</h2>
                    <div className="mb-5">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                        {[
                            ...
                            { key: 'facebook', label: 'Facebook', color: '#1877F2', placeholder: 'profile name' },
                        ].map(social => (
                            <div key={social.key}>
                                <label className="block text-xs tracking-widest uppercase mb-2" style={{ color: social.color, opacity: 0.7 }}>
                                    {social.label}
                                </label>
